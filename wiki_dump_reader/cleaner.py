...
class Cleaner:
    ...
    def _remove_image_links(self, text: str):
        """Remove links like `[[File:*]]`"""
        return self._remove_resource_links(text, "Image")

    def _remove_resource_links(self, text: str, resource):
        """Remove links likes `[[*:*]]`"""
        pattern = "[[" + resource + ":"
    ...
    def _remove_refs(self, text: str) -> str:
    ...
    def _remove_emphasises(self, text: str) -> str:
    ...
    def _remove_langs(self, text: str) -> str:
        """Remove pattenrs like {{lang-*|*}}}"""
        return re.sub(
            r"{{lang(-|\|).*?\|(.*?)}}", r"\2", text, flags=re.IGNORECASE | re.DOTALL
        )
    ...
    def _remove_choices(self, text: str) -> str:
        ...
        text = re.sub(
            r"-{.{,100}?:(.{,100}?)(;.{,100}?}-|}-)", r"\1", text, flags=re.DOTALL
        )
        text = re.sub(r"-{(.{,100}?)}-", r"\1", text, flags=re.DOTALL)
        return text

    def _remove_templates(self, text: str):
        """Remove patterns like {{*}}"""
        begin, removed = 0, ""
        while begin < len(text):
                ...
                if ch == "{":
                    ...
                    if depth == 0:
                        ...
                        if len(parts) == 1:
                            ...
                            if template_type in {"le"} or template_type.startswith(
                                "link-"
                            ):
                                removed += parts[1]
                        break
    ...
    def _remove_htmls(self, text: str) -> str:
        return re.sub(r"<(.*?)>", "", text, flags=re.DOTALL)

    def _remove_lists(self, text: str) -> str:
        return re.sub(r"^\s*[\*#]\s*", "", text, flags=re.MULTILINE)

    def _remove_indents(self, text: str) -> str:
        return re.sub(r"^\s*[:;]\s*", "", text, flags=re.MULTILINE)

    def _remove_styles(self, text: str) -> str:
        return re.sub(
            r":?{\| (style|class)=.*?\|}", "", text, flags=re.IGNORECASE | re.DOTALL
        )

    def _remove_spaces(self, text: str) -> str:
        return text.replace("\u200b", "")

    def _remove_continuous_newlines(self, text: str) -> str:
        return re.sub(r"\n{2,}", "\n", text)

    def build_links(self, text: str) -> tuple[str, list[dict[str, Any]]]:
        begin, removed, links = 0, "", list[dict[str, Any]]()
        while begin < len(text):
            pattern_begin = text.find("[[", begin)
            if pattern_begin == -1:
                if begin == 0:
